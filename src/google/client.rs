...
pub fn detect(
    runtime: &Runtime,
) -> Result<Option<GoogleClientConfig>, GoogleError> {
...
pub fn validate(
    data: &[u8],
) -> Result<GoogleClientConfig, GoogleError> {
    let credentials: GoogleCredentialsFile =
        ...
        .map_err(
            |error| {
                format!(
                    "Invalid Google client JSON: {error}"
                )
            },
        )?;
    ...
    {
        return Err(
            "Google client_id does not appear to be a valid OAuth Client ID"
                .into(),
        );
    }
...
pub fn import(
    ...
    let parent = config_path
        .parent()
        .ok_or(
            "Unable to determine Google client configuration directory",
        )?;
