...
pub fn set_boreal_home(
    ...
    let dirs = boreal
        .get_mut("BOREAL")
        .and_then(
            |value| value.get_mut("DIRS")
        )
        .and_then(
            Value::as_object_mut
        )
        .ok_or(
            "Missing BOREAL.DIRS in boreal.json",
        )?;
    ...
    while !unresolved.is_empty() {
        ...
        for name in names {
            let value = unresolved
                .get(&name)
                .expect(
                    "unresolved directory disappeared",
                );
